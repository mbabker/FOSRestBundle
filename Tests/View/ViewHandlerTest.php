...
class ViewHandlerTest extends TestCase
{
    ...
    public function testShouldReturnErrorResponseWhenDataContainsFormAndFormIsNotValid(): void
    {
        $this->serializer
            ->expects($this->once())
            ->method('serialize')
            ->will($this->returnCallback(fn($data, $format, $context): string => serialize([$data, $context])));
    ...
    {
        $viewHandler = $this->createViewHandler($formats);
        $viewHandler->registerHandler('html', fn($handler, View $view): \Symfony\Component\HttpFoundation\Response => new Response('', $view->getStatusCode()));
    ...
    public function testHandleCustom(): void
    {
        ...
        $viewHandler->registerHandler('html', (fn(): \Symfony\Component\HttpFoundation\Response => new Response('foo')));
